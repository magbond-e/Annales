  ...
  useEffect(() => {
    ...
    const loadClip = async (segment: RuntimeSegment) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Clip status: ${response.status}`);
        }
  ...
  return (
    ...
    >
      <div className="scroll-scrub__stage">
        ...
        <nav aria-label="Chapitres de découverte" className="scroll-scrub__route">
